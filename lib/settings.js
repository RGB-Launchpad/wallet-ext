...
export function migrate(stored) {
    ...
    // NOTE: a stored network the wallet no longer offers makes every unlock fail with
    // "Unknown network", so it falls back to the default instead.
    if (!Object.hasOwn(NETWORKS, raw.network)) {
    ...
    for (const [net, saved] of Object.entries(raw.byNetwork || {})) {
        if (!Object.hasOwn(NETWORKS, net)) { changed = true; continue; }
        const kept = { ...saved };
